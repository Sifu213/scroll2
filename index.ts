...
function displayTokenTaxes(tokenMetadata: any) {
  ...
  if (buyTokenBuyTax > 0 || buyTokenSellTax > 0) {
    console.log(`Buy - Buy Tax: ${buyTokenBuyTax}%`);
    console.log(`Buy - Sell Tax: ${buyTokenSellTax}%`);
  }
 
  if (sellTokenBuyTax > 0 || sellTokenSellTax > 0) {
    console.log(`Sell - Buy Tax: ${sellTokenBuyTax}%`);
    console.log(`Sell - Sell Tax: ${sellTokenSellTax}%`);
  }
}
...
const main = async () => {
  ...
  console.log("Fetching price swap 0.1 WETH for wstETH");
  console.log(
    `https://api.0x.org/swap/permit2/price?${priceParams.toString()}`
  );
  ...
  console.log("Fetching quote swap 0.1 WETH for wstETH");
  ...
  if (quote.permit2?.eip712) {
    ...
    if (signature && quote?.transaction?.data) {
      ...
      throw new Error("Failed to obtain the signature or transaction data");
    }
  }
